...
class Nette extends Command
{
    ...
    protected $signature = 'create:nette
							{name=blog}
							{--path=}';

    protected $description = 'Create Nette projects';
    ...
    public function handle(): void
    {
        ...
        $this->installer = new ComposerPackageInstaller('Nette', 'nette');
        $this->webzone = new Webzone();

        $this->webzone->clear();
        $this->webzone->logo('Nette Framework', 'comment');
        $this->init();
    }
    ...
    private function init(): void
    {
        ...
        $this->task('Setting up Installer', function () use ($path): void {
            ...
            $this->installer->setComposerPackage('nette/web-project');
        });
    ...
    {
        $this->newline();
        $this->info('Creating Nette Application...');
        $this->newline();
        $this->installer->install();
        $this->newline();
        $this->info("  Nette app created successfully at {$this->installer->mainPath}/{$this->installer->name}. ");
        $this->newline();
